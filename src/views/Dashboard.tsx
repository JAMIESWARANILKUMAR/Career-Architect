  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-6 grid-rows-2 gap-4 h-auto sm:h-[420px]">
        ...
        >
          ...
          <div className="flex justify-between items-start gap-8">
            <div className="flex-1">
              <h3 className="text-3xl font-light text-white tracking-tight mb-4 leading-none">
                Quadrant <span className="text-blue-400 font-bold">Alpha</span> Match
              </h3>
              <p className="text-sm text-slate-400 leading-relaxed font-medium max-w-[90%] italic border-l-2 border-blue-500/30 pl-4 mb-8">
                "Gemini Protocol: High-yield convergence detected in <span className="text-white font-bold underline decoration-blue-500/50">Full-Stack Synthesis</span> nodes."
              </p>
            </div>
            <div className="hidden sm:flex flex-col items-center gap-1 bg-white/[0.03] border border-white/5 p-4 rounded-3xl">
              <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest">Match Prob.</span>
              <span className="text-3xl font-mono font-light text-blue-400 tracking-tighter">94.2<span className="text-[10px]">%</span></span>
            </div>
          </div>
        ...
        <motion.div 
          variants={item} 
          whileHover={{ y: -5, backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(59, 130, 246, 0.2)' }}
          className="col-span-3 row-span-1 glass-card !p-6 flex flex-col justify-between group transition-all shadow-xl relative overflow-hidden border-white/5"
        >
           <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <div className="w-16 h-16 border-2 border-blue-500 rounded-full animate-[spin_10s_linear_infinite]" />
           </div>
           <div className="w-10 h-10 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-400 mb-4 group-hover:scale-110 transition-transform">
             <Briefcase size={20} />
           </div>
           <div>
             ...
             <div className="flex items-baseline gap-2">
               <AnimatePresence mode="wait">
                 <motion.span
                   ...
                   className="text-3xl font-mono font-light text-white tracking-tighter"
                 >
                   {metrics.applicationsCount.toString().padStart(2, '0')}
                 </motion.span>
               </AnimatePresence>
               <span className="text-[8px] text-emerald-400 font-black uppercase px-2 py-0.5 bg-emerald-500/10 rounded-full border border-emerald-500/10">Active</span>
             </div>
           </div>
        </motion.div>
        ...
        <motion.div 
          variants={item} 
          whileHover={{ y: -5, backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(245, 158, 11, 0.2)' }}
          className="col-span-3 row-span-1 glass-card !p-6 flex flex-col justify-between group transition-all shadow-xl relative overflow-hidden border-white/5"
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
          ...
          <div className="w-10 h-10 bg-amber-500/10 rounded-2xl flex items-center justify-center text-amber-500 mb-4 group-hover:scale-110 transition-transform">
           ...
           <div>
             <span className="text-[9px] text-slate-500 uppercase tracking-[0.2em] font-black block mb-1">Daily Sequence</span>
             <div className="flex items-baseline gap-2">
               <span className="text-3xl font-mono font-light text-white tracking-tighter">{metrics.dailyStreak.toString().padStart(2, '0')}</span>
               <span className="text-[8px] text-amber-400 font-black uppercase px-2 py-0.5 bg-amber-500/10 rounded-full border border-amber-500/10 tracking-widest">Locked</span>
             </div>
           </div>
        </motion.div>
      </div>
